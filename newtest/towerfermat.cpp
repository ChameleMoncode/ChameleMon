    ...
    for (int times = 0; times < TIMES; times++)
    {
        ...
        for (int iter_data = 0; iter_data < 2; ++iter_data)
        {
            ...
            for (itr = fermattower_hh.begin(); itr != fermattower_hh.end(); ++itr)
            {
                ...
                // if (HH_true.find(*itr) == HH_true.end())
                // {
                //     printf("<%x, %d>\n", *itr, true_freq[*itr]);
                //     fermat_tower->towerfilter->query((const char *)&(*itr));
                //     printf("<%5d, %5d>\n", fermat_tower->towerfilter->idx[0], fermat_tower->towerfilter->idx[1]);
                // }
            }
            for (itr = HH_true.begin(); itr != HH_true.end(); ++itr)
            {
                ...
                // cout << true_freq[key] << "  "<< fermat_tower->query((const char *)&key) << "  "
                //  << std::abs((double)((int)true_freq[key] - (int)fermat_tower->query((const char *)&key))) / (double)true_freq[key] <<endl;
            }
            // cout << HH_are << "/" << HH_true.size()<<endl;
            HH_are /= HH_true.size();
